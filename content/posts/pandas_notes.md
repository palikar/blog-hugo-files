...
keywords = ["machile-learning", "python", "numpy"]
lastmod = 2018-08-01T00:58:16+02:00
...
My note while reading [this](https://www.tutorialspoint.com/numpy/index.htm). More or less copy paste, Take with a grain of salt...or sugar.
...
The object can be later used when a given function or constructor taked _dtype_ argument. Those object can also create struct-like types for structured data. Think of it like defining custom struct in C++ and then creating a vector with elements of this struct.
...
This way we can define map-like structure that contains a bunch of different attays, each adressable with a key. The following code defines a 'human' data type with _string_ field _name_, _int_ field age, and _float_ field social class.
...
_Shape_ of an array is its layout in memory. Normally _numpy_ represents the ndarray as a n-dimensional array in memory. This means that that the shapes is nothing more than a tupple that consists of each dimension's size. For example, the array `[1,2,3]` has shape `(3)`, the array `[[1,2],[1,2]]` has shape `(2,2)` because it consists of two array and each of them contains two elements.
...
The elements stay the same and occupy the same memory location but the acces to them is happening in different way.\*\* 'Size' of an array **numpy.itemsize** - this array attribute returns the length of each element of array in bytes.
...
The attribute that eveyone has been waiting for... **numpy.size**
