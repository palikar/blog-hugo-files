...
keywords = ["machile-learning", "python", "pandas"]
lastmod = 2018-07-26T12:45:26+02:00
categories = ["machile-learning"]
draft = false
weight = 100
+++
...
**pandas** is a Python package providing fast, flexible, and expressive data structures designed to make working with "relationaHeterogeneous datal" or "labeled" data both easy and intuitive. It aims to be the fundamental high-level building block for doing practical, **real world** data analysis in Python. Additionally, it has the broader goal of becoming the most powerful and flexible open source data analysis / manipulation tool available in any language. It is already well on its way toward this goal. <br /> It's lika a data-base-like objects to deal with your data.
...
-   Columns are atributes
-   Rows are the actuall data
-   Indecies are the lables for each row

This is the most common onject when dealing with data and using _pandas_. The data in the object is heterogeneous, the shape and the size are mutable. The last part means that the models you build in one object that contains data can easly(ish) be transformed into another model(by model I mean the way your data is structured- names and count of columns(atributes), indexing of the rows, type of the data inside). <br /> Basic construction of DataFrame object is as follows:
...
All of the constructions are pretty intuitive and easy to understand. The thing to remember - all of the possible way that you can construct _DataFrame_. <br /> The column slections is just as easy:
...
Sellection of rows either by index or by lable (the thing that was in the **index** attribute in the constructor)
...
From what I undertand, this is not widely used and I don't think I need it for my project so...fuck it. <span class="underline">Skip!!!</span>
...
-   axes : eturns a list with the row axis labels and column axis labels as the only members.
...
..ok, kinda. The table is from me and I am kinda showing off...so yeah.
...
Note how the **np.mean** return a sigle number so in the final result there is only one row - the mean of each column.


### ApplyMap {#applymap}

Not all functions can be vectorized (neither the NumPy arrays which return another array nor any value), the methods applymap() on DataFrame and analogously map() on Series accept any Python function taking a single value and returning a single value. This is similar to the **pipe** but it's less flexible. It just treats the whole _DataFrame_ as on big list and performes a mappint function on it. Example:
...
There are two possablities for sorting:
...
There are some custom way slicing through data that are optimized and are the remended way of slicing data when dealing with production code.
...
1.  **loc()** - lable based indexing. Used as df.loc[</rows/>,</columns/>]. For _rows_ and _columns_ could be given pretty much everything that makes sense - single char, list of labels, slice object, boolean array..
    ...
    df.loc[['a','b','f','h'],['A','C']]
    ```
2.  **iloc()** - index based indexing. Used the same way as **loc()** but just with integer indecies.
...
## Thougher statistics {#thougher-statistics}


### Some functions {#some-functions}

There are some useful statistical funcitons already in _pandas_ that help with the understanding and analyzing the behavior of data
...
The first row is _NaN_ because ther is no previous element to copare it to.
...
_NaN_ values are ignored automagically.
...
A lot of times some string-fields in the data aren't just some random text but a repetetive and en element of some predefiened set ot possible values. Those are the categorical types of data. Something like [big, medium, small]. Categorical variables can take on only a limited, and usually fixed number of possible values. Besides the fixed length, categorical data might have an order but cannot perform numerical operation. Categorical are a _Pandas_ data type. <br /> A simple example to create a _Series_ object that only can contain [a, b, c]
...
If _categories_ aren't provided, they are inffered from the values. _ordered_ specifies wather or not on categories is bigger or not than other.
...
<br /> **obj.cat.categories** command is used to get the categories of the object. <br /> Removing categories is also something that comes in handny and of course it's possibe with _pandas_
...
Skipping rows can be achieved through the _skiprows=n_ aegument of **read\_cvs()**
