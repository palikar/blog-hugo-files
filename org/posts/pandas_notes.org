...
#+HUGO_TAGS: 
#+HUGO_CATEGORIES: machile-learning
#+HUGO_WEIGHT: 100


#+BEGIN_NOTES
For transperanty sake - those are my notes while learning about /pandas/ from [[https://www.tutorialspoint.com/python_pandas/index.htm%20][this]] tutorial. At times this here is just copy paste from it, at other it's my own thoughts and explanations. This is meant to be a condensed version of the tutorial more or less just for me. If you found it useful though....well, good ofor you, I guess.
#+END_NOTES
 
* Abstract
** Basic
*pandas* is a Python package providing fast, flexible, and expressive data structures designed to make working with "relationaHeterogeneous datal" or "labeled" data both easy and intuitive. It aims to be the fundamental high-level building block for doing practical, *real world* data analysis in Python. Additionally, it has the broader goal of becoming the most powerful and flexible open source data analysis / manipulation tool available in any language. It is already well on its way toward this goal.
\\
It's lika a data-base-like objects to deal with your data.
...
\\
The column slections is just as easy:
#+BEGIN_SRC python
...
#+END_SRC
** DataPanel
From what I undertand, this is not widely used and I don't think I need it for my project so...fuck it. _Skip!!!_
* Basic usage
...
* Thougher statistics
** Some functions
There are some useful statistical funcitons already in /pandas/ that help with  the understanding and analyzing the behavior of data
...
Gives us:
   ...
   4 -1.548769  -9.686350
   #+end_example
   The first row is /NaN/ because ther is no previous element to copare it to.
...
: -0.23077206068332465
/NaN/ values are ignored automagically.
...
The second step could have some vartiations. Maybe we can want to transform or filter the data but the genaral idea stays. Pandas offers some great functions to achieve all of this.
\\
...
#+END_EXAMPLE
* Categories
A lot of times some string-fields in the data aren't just some random text but a repetetive and en element of some predefiened set ot possible values. Those are the categorical types of data. Something like [big, medium, small]. Categorical variables can take on only a limited, and usually fixed number of possible values. Besides the fixed length, categorical data might have an order but cannot perform numerical operation. Categorical are a /Pandas/ data type.
\\
...
#+END_EXAMPLE
If /categories/ aren't provided, they are inffered from the values. /ordered/ specifies wather or not on categories is bigger or not than other.
#+BEGIN_SRC python :results output
import pandas as pd
...
\\
Removing categories is also something that comes in handny and of course it's possibe with /pandas/
#+BEGIN_SRC python :results output
import pandas as pd
...
print (df)
#+END_SRC
Skipping rows can be achieved through the /skiprows=n/ aegument of *read_cvs()*
