...
#+END_QUOTE
From what I understand - One would used that when applying a bunch of functions on all elements of /DataFrame/ (or whatever) while still having the possibility that the applied functions takes more than just one argument. For example, let's add two to every element in /DataFrame/ through a adder functions that just adds its two arguments.
#+BEGIN_SRC python
import pandas as pd
import numpy as np
